...
class MyBot(sc2.BotAI):
    def __init__(self, use_model=True):
        ...
        if(self.use_model):
            self.model=keras.models.load_model("CNN.0001-LR-STAGE1")
    ...
    async def intel(self):
        ...
        basic_units = ["nexus", "commandcenter", "hatchery", "probe", "scv", "drone"]
        for structure in self.known_enemy_structures:
            pos = structure.position
            if structure.name.lower() not in basic_units:
                cv2.circle(game_data, (int(pos[0]), int(pos[1])), 5, (200, 50, 212), -1)
            else:
                cv2.circle(game_data, (int(pos[0]), int(pos[1])), 15, (200, 50, 212), -1)

        for enemy in self.known_enemy_units:
            pos = enemy.position
            if enemy.name.lower() not in basic_units:
    ...
    async def expand(self):
        if self.units(NEXUS).amount < (self.iteration / self.ITERATIONS_PER_MINUTE) and self.can_afford(NEXUS) and len(self.units(PROBE)) > 0:
    ...
    async def attack(self):
        if len(self.units(VOIDRAY).idle) > 0:
            ...
            print(selected_choice[choice])
            #no attack
            #attack unit closest to base
            #attack known enemy buildings
            #attack enemy spawn
            # unit: [#to fight, #todefend]
            target = False
            if self.iteration > self.do_something_after:
                ...
                elif choice == 1: #attack closest unit
                    if len(self.known_enemy_units) > 0:
